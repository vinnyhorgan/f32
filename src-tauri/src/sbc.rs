...
// Global peripheral state for MMIO hooks
// Using thread-local storage for the SBC peripherals
thread_local! {
    static SBC_UART: RefCell<Option<Rc<RefCell<Uart16550>>>> = const { RefCell::new(None) };
    static SBC_CFCARD: RefCell<Option<Rc<RefCell<CfCard>>>> = const { RefCell::new(None) };
}
...
fn sbc_read_hook(address: u32) -> Option<u8> {
    match decode_address(address) {
        SbcAddressRegion::Uart(offset) => SBC_UART.with(|uart_cell| {
            uart_cell
                .borrow()
                .as_ref()
                .map(|uart| uart.borrow_mut().read(offset))
        }),
        SbcAddressRegion::CfCard(offset) => SBC_CFCARD.with(|cf_cell| {
            cf_cell
                .borrow()
                .as_ref()
                .map(|cf| cf.borrow_mut().read(offset))
        }),
...
fn sbc_write_hook(address: u32, value: u32, size: OperandSize) -> WriteHookResult {
    match decode_address(address) {
        SbcAddressRegion::Uart(offset) => {
            SBC_UART.with(|uart_cell| {
                if let Some(uart) = uart_cell.borrow().as_ref() {
                    match size {
                        OperandSize::Byte => {
        ...
        SbcAddressRegion::CfCard(offset) => {
            SBC_CFCARD.with(|cf_cell| {
                if let Some(cf) = cf_cell.borrow().as_ref() {
                    match size {
                        OperandSize::Byte => {
...
impl Sbc {
    ...
    pub fn new() -> Self {
        ...
        // Register peripherals in thread-local storage
        SBC_UART.with(|cell| {
            *cell.borrow_mut() = Some(Rc::clone(&uart));
        });
        SBC_CFCARD.with(|cell| {
            *cell.borrow_mut() = Some(Rc::clone(&cfcard));
        });
    ...
    ///
    /// Also initializes system variables normally set by ROM boot:
    /// - OUTCH_VEC = pointer to uart_outchar routine
    /// - INCH_VEC = pointer to uart_inchar routine
    /// - HEXDIGITS = pointer to hex digit table
    /// - SEPARATORS = formatting separator characters
    pub fn run_app(&mut self) {
        // Initialize system variables that ROM would set during boot
        // These are required for syscalls like OutChar/OutStr to work
        let _ = self.cpu.memory.write_long(OUTCH_VEC, UART_OUTCHAR_ADDR);
        let _ = self.cpu.memory.write_long(INCH_VEC, UART_INCHAR_ADDR);
        let _ = self.cpu.memory.write_long(HEXDIGITS_VEC, HEXDIGITS_UC_ADDR);
        let _ = self.cpu.memory.write_long(SEPARATORS_VEC, SEPARATORS_VALUE);
